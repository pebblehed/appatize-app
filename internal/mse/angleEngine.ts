...
function describeBehaviourControls(behaviour?: BehaviourControlsInput): string {
  ...
  if (behaviour.energy) {
    // Note: BehaviourEnergy includes legacy labels. We normalize to AngleEnergy labels for prompt text.
  ...
  if (behaviour.platformBias) {
    ...
    if (p === "tiktok") {
      ...
      parts.push("Platform bias: Instagram Reels — slightly more polished visual tone.");
    } else if (p === "shorts") {
      parts.push(
 ...
 * (Removes the need for explicit `any` while keeping behaviour stable.)
 */
...
function heuristicVariantConfidence(v: {
  ...
  if (cta && normalizeWhitespace(cta).length >= 8 && !ctaLower.includes("follow for more"))
    score += 0.03;
...
function hardenResult(
  ...
  // Treat incoming angles as unknown, then harden deterministically.
  const angles: unknown[] = Array.isArray(getObjProp(parsed, "angles"))
    ? ((getObjProp(parsed, "angles") as unknown[]) ?? [])
    : [];

  // Normalize angles
  const hardenedAngles = angles
    .map((a, idx) => {
      const angleId = normalizeId(getObjProp(a, "id"), `angle-${idx + 1}`);

      // Normalize title
      let title = normalizeAngleTitle(safeStr(getObjProp(a, "title")));
      ...
      const variantsRaw = getObjProp(a, "variants");
      const variants: unknown[] = Array.isArray(variantsRaw) ? variantsRaw : [];

      const hardenedVariants = variants
        .map((v, vIdx) => {
          const parentAngleId = angleId;

          const variantId = normalizeId(getObjProp(v, "id"), `${angleId}-variant-${vIdx + 1}`);

          const hook = normalizeWhitespace(safeStr(getObjProp(v, "hook")));
          const body = normalizeWhitespace(safeStr(getObjProp(v, "body")));
          const cta = normalizeWhitespace(safeStr(getObjProp(v, "cta")));
          const outro = normalizeWhitespace(safeStr(getObjProp(v, "outro")));
          const structureNotes = normalizeWhitespace(safeStr(getObjProp(v, "structureNotes")));

          const confRaw = getObjProp(v, "confidence");
          const modelConf = clamp01(typeof confRaw === "number" ? confRaw : 0);
          ...
          // Preserve original fields only if it's an object (bounded; avoids spreading unknown).
          const vObj = v && typeof v === "object" ? (v as Record<string, unknown>) : {};
      ...
      // Preserve original angle fields only if it's an object (bounded).
      ...
      return {
        ...
        platform:
          safeStr(getObjProp(a, "platform")) || safeStr(parsed?.angles?.[idx]?.platform) || "",
  ...
  return {
    ...parsed,
    angles: (dedupedAngles.length >= 3
      ? dedupedAngles
      : hardenedAngles) as ScriptGenerationResult["angles"],
 ...
 */
function assertValidShape(parsed: unknown) {
  if (!parsed || typeof parsed !== "object") {
    throw new Error("Response is not an object");
  }

  const angles = getObjProp(parsed, "angles");
 ...
 * High-level read on the cultural moment and how to play this moment.
 */
