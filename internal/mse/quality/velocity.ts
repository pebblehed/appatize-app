...
function mapRatioToScore(ratio: number, recentAvg: number, used: number): number {
  ...
  let base: number;
  if (ratio < 1) base = 0.2;
  else if (ratio < 1.5) base = 0.35 + (ratio - 1) * 0.4;
  else if (ratio < 2) base = 0.55 + (ratio - 1.5) * 0.3;
  else if (ratio < 3) base = 0.7 + (ratio - 2) * 0.15;
  else if (ratio < 4) base = 0.85 + (ratio - 3) * 0.1;
  else base = 1.0;

  const activityFactor = clamp01(recentAvg / 2);
  const sampleFactor = clamp01(used / 6);

  const dampened = base * (0.5 + 0.5 * activityFactor) * (0.6 + 0.4 * sampleFactor);
