...
/* ------------------------------------------------------------------ */
/* Safe helpers — remove explicit `any` while preserving behaviour    */
/* ------------------------------------------------------------------ */
...
function asId(v: unknown): string | number | undefined {
  return typeof v === "string" || typeof v === "number" ? v : undefined;
}

function asCreatedAt(v: unknown): string | number | Date | undefined {
  return typeof v === "string" || typeof v === "number" || v instanceof Date ? v : undefined;
}
...
export function qualifyMoment(
  ...
  const velocity = computeVelocity(
    signals
      .map((s) => ({
        createdAt: asCreatedAt(getObjProp(s, "createdAt")),
      }))
      .filter((s): s is { createdAt: string | number | Date } => s.createdAt !== undefined),
    opts.velocity
  );
  ...
  const legibilityInput: LegibilityInput = {
    text: legibilityText,
...
function extractKeywords(signals: MomentCandidate["signals"]): string[] | undefined {
  ...
  for (const sig of signals) {
    const arr = getObjProp(sig, "keywords");
...
function computeMaturityFromSignals(args: {
