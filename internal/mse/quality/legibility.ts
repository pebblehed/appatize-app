...
export function computeCulturalLegibility(input: LegibilityInput): LegibilityResult {
  ...
  const soupPenalty = clamp01(1 - (0.7 * allCapsRatio + 0.4 * digitRatio));
  const vocabBonus = clamp01(1.15 - avgWordLen / 8);

  const score = clamp01(
    (0.45 * lengthScore + 0.35 * complexityPenalty + 0.2 * soupPenalty) * (0.8 + 0.2 * vocabBonus)
  );
...
function scoreLength(wordCount: number, charLen: number): number {
  ...
  return clamp01(wcScore * (0.7 + 0.3 * charPenalty));
}
