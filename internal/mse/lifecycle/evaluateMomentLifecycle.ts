...
import type { MomentMemoryRecord } from "../../contracts/MOMENT_MEMORY_RECORD";
...
function computeICS(memory: MomentMemoryRecord, signals: SignalItem[]) {
  ...
  const canonicalKeywords = safeArray(mem.canonical?.signatureKeywords);
  const canonicalEntities = safeArray(mem.canonical?.anchorEntities);
...
export function evaluateMomentLifecycle(args: {
  ...
  const canonKw = safeArray(mem.canonical?.signatureKeywords);
  const canonEn = safeArray(mem.canonical?.anchorEntities);
